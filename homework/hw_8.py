...
def get_books_by_autor(autor):
    result =  conn.execute("""
     SELECT * FROM books WHERE autor = ? ORDER BY name ASC """,(autor,))
    return result.fetchall()
...
if __name__ == "__main__":
    ...
    # delete_book(conn,3)
    print(get_books_by_autor(autor="Антуан"))
    print(get_books_by_autor(autor="Льюис"))
    print(get_books_by_autor(autor="Виктор"))
    conn.close()
